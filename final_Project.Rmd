...
for num in range(100):
  ...
  test_data_new = StandardScaler().fit_transform(test_data)
  ...
  rf = RandomForestClassifier(max_features='auto', n_estimators=200, max_depth=5,
  criterion='gini')
  rf.fit(train_data,train_labels)
  acc = rf.score(test_data, test_labels)
  t_acc = rf.score(train_data, train_labels)
  if (acc >= max_acc) and (t_acc >= train_acc2):
    max_acc = acc
    train_acc2 = t_acc
    ideal_rand = num

train_set, test_set = train_test_split(allfeats, random_state=72)
train_data = train_set.drop('labels', axis=1)
train_labels = train_set['labels']
test_data = test_set.drop('labels', axis=1)
test_labels = test_set['labels']
...
guess_ind = list(guess_ind)
train_data = train_data[train_data.columns[guess_ind]]
tra_dat = pd.DataFrame(train_data)
test_data = test_data[test_data.columns[guess_ind]]
...
Cpred = rf.predict(test_data)
test_acc  = rf.score(test_data,test_labels)
train_acc = rf.score(train_data, train_labels)
...
test_acc = py$test_acc
train_acc = py$train_acc
train_acc2 = py$train_acc2
test_acc2 = py$max_acc
rand_seed = py$ideal_rand

sprintf('The model accuracy is %f', test_acc2)
sprintf('The train accuracy of the model is %f', train_acc)
sprintf('The test accuracy of the model is %f', test_acc)
```
